function [quest] = format_quest(Qstring, ResX, PixPerChar, Xoffset)
Xoffset=Visual.offsetX
spaces= strfind(Qstring, ' '); 
usable_scr= ResX-  2*Xoffset; 
usable_scr_letters= usable_scr/ PixPerChar;
n_line= length(Qstring)/ usable_scr_letters; % number of lines to fit text
...
for i=1:n_line    
    if i==1
        ...
        if i== n_line && length(Qstring) < breaked+usable_scr_letters 
            tmp_txt= [Qstring(breaked+1:end) '' '\n'];
        elseif i== n_line && length(Qstring) > breaked+usable_scr_letters
            tmp_txt= [Qstring(breaked+1:line_break) '' '\n' Qstring(line_break+2:end)];
        else
            tmp_txt= [Qstring(breaked+1:line_break) '' '\n'];
        end
