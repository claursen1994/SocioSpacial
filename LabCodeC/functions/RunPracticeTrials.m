% Presentation of Experimental Practice trials
% Martin Vasilev, 2017
...
% stimuli set-up:
    ...
    if cond< 5 % Social 1st (cond 1 - 4)
        whichRow= find(PracSent.item== item & PracSent.cond== cond, 1);
    else % Spatial 1st (cond 5 - 8)
        whichRow= find(PracSent.item== item & PracSent.cond== cond, 1); 
    end
   

    P1_text= char(PracSent.P1(whichRow));
    P1_text = format_text(P1_text , Visual.resX, Visual.Pix_per_Letter, Visual.offsetX);
    P2_text= char(PracSent.P2(whichRow));
...
%     sentenceString=strjoin(sentenceString,sentenceString2)
 ...
 my_indexer = ((item-1) * 8) + cond; % my_indexer picks the row
    ...
    options= [ '1)  ' char(PracQuest.Q1o1(my_indexer)) '/n' '2)  ' char(PracQuest.Q1o2(my_indexer)) '/n' '3)  ' char(PracQuest.Q1o3(my_indexer))];
    options= strjoin(strsplit(options, '"'));    
    question= char(PracQuest.Q1(my_indexer));
%     question= strjoin(strsplit(question, '"'));
%     question= format_text(question , Visual.resX, Visual.Pix_per_Letter, Visual.offsetX);
    question= strjoin(strsplit(question, '"'));

    answer1= QuestionMC(question, strsplit(options, '/n'), PracQuest.Q1corr_ans(my_indexer), item, cond, 1);
    
    % Question 2 ? for eyelink?
    options= [ '1)  ' char(PracQuest.Q2o1(my_indexer)) '/n' '2)  ' char(PracQuest.Q2o2(my_indexer)) '/n' '3)  ' char(PracQuest.Q2o3(my_indexer))];
    options= strjoin(strsplit(options, '"'));     
    question= char(PracQuest.Q2(my_indexer));
    question= strjoin(strsplit(question, '"'));
%     question= format_text(question , Visual.resX, Visual.Pix_per_Letter, Visual.offsetX);
%     question= strjoin(strsplit(question, '"'));
    answer2= QuestionMC(question, strsplit(options, '/n'), PracQuest.Q2corr_ans(my_indexer), item, cond, 2);
    
    % Question 2 ? for eyelink?
    options= [ '1)  ' char(PracQuest.Q3o1(my_indexer)) '/n' '2)  ' char(PracQuest.Q3o2(my_indexer)) '/n' '3)  ' char(PracQuest.Q3o3(my_indexer))];
    options= strjoin(strsplit(options, '"'));    
    question= char(PracQuest.Q3(my_indexer));
    question= strjoin(strsplit(question, '"'));
%     question= format_text(question , Visual.resX, Visual.Pix_per_Letter, Visual.offsetX);
%     question= strjoin(strsplit(question, '"'));
    answer3= QuestionMC(question, strsplit(options, '/n'), PracQuest.Q3corr_ans(my_indexer), item, cond, 3);
    
    end


% end of Experiment text:
text= 'The Practice is finished.';
